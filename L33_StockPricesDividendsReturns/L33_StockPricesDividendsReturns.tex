...
\begin{frame}{\ftheader{Stocks Prices, Dividends, and Returns}}
	\begin{itemize}
		\item The CAPM gives us a way of computing a stock's return
...
\begin{frame}{\fttheory{Stocks}}
	...
	\begin{itemize}
		...
		\item $r_{S}$ is the required total rate of return 
...
\begin{itemize}
	...
	\begin{equation*}
		\overbrace{\frac{D_{1}}{P_{0}}+g}^{\text{Specific Formula}}\should r_{S}\should\overbrace{\underbrace{\frac{D_{1}}{P_{0}}}_{\text{Dividend Yield}}+\underbrace{\frac{P_{1}-P_{0}}{P_{0}}}_{\text{Capital Gain Rate}}}^{\text{General Formula}}
	\end{equation*}
\end{itemize}
\end{frame}
...
\begin{align}
	...
	&=\frac{D_{1}}{\beta_{g}\text{E}[r_{m}-r_{f}]-\text{E}[g-r_{f}]}
\end{align}
	where $D_{1}=(1+\text{E}[g]-\beta_{g}\text{E}[r_{m}-r_{f}])D_{0}$
\end{frame}
\end{document}

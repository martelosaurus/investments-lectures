...
\begin{frame}{\ftheader{Lecture 0.1: Introduction}}
	\begin{itemize}
		...
		\item Asset classes and the markets in which they trade
		\item Basic market operations (shorting, leverage, arbitrage)
	\end{itemize}
\end{frame}
...
\begin{frame}{\fttheory{NPV Revisited}}
	\begin{itemize}
		...
		\begin{equation}
			\text{NPV}=-.75\musd+\frac{1\musd}{1.02}.
		\end{equation}
...
\begin{frame}{\fttheory{Q1: What if the NPV is negative?}}
	\begin{itemize}
		\item You can't sell a factory that you don't own, but you \textit{can} sell a stock/bond/derivative that you don't own
		\item Just ``borrow'' the security from someone (Sally), sell it for $.75\musd$, invest the proceeds at $2\%$; in one year, pay Sally $1\musd$
		\item In this case,
		\begin{equation}
			\text{NPV}=.75\musd-\frac{1\musd}{1.02}>0
		\end{equation}
	\end{itemize}
\end{frame}

\begin{frame}{\fttheory{Q2: What if you finance your investment with debt?}}
	\begin{itemize}
	\item In this case,
		\begin{equation}
			\text{NPV}=-.75\musd+\frac{1\musd}{1.02}.
		\end{equation}
	\item (what if instead of $1\musd$, the investment only produces $0.5\musd$?)
	\end{itemize}
\end{frame}

\begin{frame}{\fttheory{Q3: What if different terms have different yields?}}
	\begin{itemize}
	\item In this case,
		\begin{equation}
			\text{NPV}=-.75\musd+\frac{1\musd}{1.01}+\frac{1\musd}{1.03^2}.
		\end{equation}
	\end{itemize}
\end{frame}

\begin{frame}{\fttheory{Q4: What if cash-flows are risky?}}
	\begin{itemize}
	\item Tesla is much riskier than Kroger 
	\item In this case,
		\begin{equation}
			\text{NPV}=-.75\musd+\frac{???\musd}{1.02}.
		\end{equation}
	\end{itemize}
\end{frame}

\begin{frame}{\fttheory{Q5: What about the economy?}}
	\begin{itemize}
	\item Tiffany's (TIF) cash-flows are much more correlated with the economy than are those of Wal-Mart (WMT). 
	\item In this case,
		\begin{equation}
			\text{NPV}=-.75\musd+\frac{1\musd}{1.02}.
		\end{equation}
...
\begin{frame}{\fttheory{Q6: What if everyone wants the investment?}}
	\begin{itemize}
	\item In this case,
		\begin{equation}
			\text{NPV}=-.75\musd+\frac{1\musd}{1.02}>0.
		\end{equation}
	\item If someone invested $85\musd$ instead of $75\musd$, their NPV would still be positive
	\item In fact, if someone invested $99.9\musd$, their NPV would still be positive!
	\item This observation will enable us to compute the prices of all sorts of financial securities, including forwards (futures) and options
	\end{itemize}
\end{frame}
...
\end{center}
\pause
If you preferred \textbf{Bet A} to bet \textbf{Bet B} you are \underline{risk-averse}, \textbf{Bet B} to bet \textbf{Bet A}, you are \underline{risk-loving}, while if you were indifferent, you are \underline{risk-neutral}
\end{frame}
...
%============================================================================% 
\section{Shorting}
\begin{frame}{\ftdata{Shorting: Implementation}}
\begin{itemize}

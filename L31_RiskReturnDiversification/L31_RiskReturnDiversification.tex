...
\begin{frame}{\fttheory{Mean and Volatility}}
	\begin{itemize}
		...
		\begin{align*}
			...
			&=(x_{A}\text{SD}[R_{A}])^{2}+(x_{B}\text{SD}[R_{B}])^{2}+2x_{A}x_{B}\text{SD}[R_{A}]\text{SD}[R_{B}]\text{Corr}[R_{A},R_{B}]
		\end{align*}
		}
...
\begin{frame}{\ftexampl{Apple and P\&G}}
	...
	\pause
	\begin{itemize}
		\item {\color{blue}Expected returns are 3.56\% and 1.09\% (resp.) and volatilities are 12.21\% and 5.21\% (resp.)}
		\item {\color{blue}-0.0541 (weird!)}
		\item {\color{blue}2.33\% and 6.51\%}
	\end{itemize}
\end{frame}
...
\begin{frame}{\ftexampl{Moral \#1}}
	...
	\begin{align*}
		\text{Var}[R_{P}]&=x_{A}^{2}\text{SD}[R_{A}]^{2}+x_{B}^{2}\text{SD}[R_{B}]^{2}\\
